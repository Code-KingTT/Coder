...
public class AuthServiceImpl implements AuthService {
        ...
        try {
            ...
            if (userResult != null && userResult.isSuccess() && userResult.getData() != null) {
                ...
                if (StrUtils.isNotBlank(existingToken) && !jwtUtils.isTokenExpired(existingToken)) {
                    ...
                    // 记录登录成功的隐私信息
                    loginResult = 1;
                    recordLoginPrivacy(userId, request, loginResult, System.currentTimeMillis() - startTime);
                    // 构建登录响应
     ...
     */
    private void recordLoginPrivacy(Long userId, HttpServletRequest request, Integer loginResult, Long loginDuration) {
        try {
            if (request != null) {
                userPrivacyService.recordLoginPrivacy(userId, request, loginResult, loginDuration);
            }
        } catch (Exception e) {
            log.error("记录登录隐私信息失败", e);
            // 隐私记录失败不影响登录流程
        }
    }
