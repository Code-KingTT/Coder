...
(
    ...
    `real_ip`              VARCHAR(50)           DEFAULT NULL COMMENT '真实IP地址（X-Real-IP）',
    `forwarded_for`        VARCHAR(200)          DEFAULT NULL COMMENT '转发IP链（X-Forwarded-For）',
    `proxy_ip`             VARCHAR(50)           DEFAULT NULL COMMENT '代理IP地址',

    -- 浏览器信息（可通过User-Agent解析）
    `user_agent`           TEXT                  DEFAULT NULL COMMENT '完整User-Agent字符串',
    `browser_name`         VARCHAR(50)           DEFAULT NULL COMMENT '浏览器名称',
    `browser_version`      VARCHAR(50)           DEFAULT NULL COMMENT '浏览器版本',
    `browser_engine`       VARCHAR(50)           DEFAULT NULL COMMENT '浏览器引擎',
    `accept`               VARCHAR(500)          DEFAULT NULL COMMENT 'Accept请求头',
    `accept_language`      VARCHAR(100)          DEFAULT NULL COMMENT 'Accept-Language请求头',
    `accept_encoding`      VARCHAR(100)          DEFAULT NULL COMMENT 'Accept-Encoding请求头',

    -- 操作系统信息（可通过User-Agent解析）
    ...
    -- 设备信息（可通过User-Agent解析）
    `device_type`          VARCHAR(20)           DEFAULT NULL COMMENT '设备类型：PC-电脑，MOBILE-手机，TABLET-平板，UNKNOWN-未知',
    ...
    `host`                 VARCHAR(100)          DEFAULT NULL COMMENT 'Host请求头',
    `origin`               VARCHAR(200)          DEFAULT NULL COMMENT 'Origin请求头',

    -- 登录相关
    `login_type`           VARCHAR(20)  NOT NULL DEFAULT 'WEB' COMMENT '登录类型：WEB-网页，APP-应用，API-接口',
    `login_result`         TINYINT(1)   NOT NULL DEFAULT 1 COMMENT '登录结果：0-失败，1-成功',
    `login_duration`       BIGINT(20)            DEFAULT NULL COMMENT '登录持续时间（毫秒）',

    -- 扩展信息
    `request_headers`      TEXT                  DEFAULT NULL COMMENT '完整请求头信息（JSON格式）',
